...
function TrusteeAssetCard({ asset, onDisbursed }) {
  ...
  async function handleGenerateReport() {
    ...
    setAiError(null);
    try {
      const res = await fetch(`${API_URL}/api/assets/${asset.id}/ai-report`);
      const data = await res.json();
      ...
      <div className="p-6">
        ...
        {aiLoading && (
          <div className="flex items-center gap-2 text-sm text-ink-soft">
            ...
            Reading the ledger with Gemini...
          </div>
        )}
            ...
            {aiReport.flags.length > 0 ? (
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
                <h5 className="mb-2 text-xs font-semibold uppercase tracking-wider text-amber-800">
                  Compliance check — {aiReport.complianceCheck}
                </h5>
              ...
              <div className="flex items-start gap-2 rounded-xl border border-zellige/25 bg-porcelain p-4">
                ...
                <p className="text-sm text-ink">
                  <span className="font-semibold text-zellige">Compliance check:</span>{" "}
                  {aiReport.complianceCheck || "No issues found"}
                </p>
              </div>
            )}
          </div>
